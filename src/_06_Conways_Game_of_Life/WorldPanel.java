...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public WorldPanel(int w, int h, int cpr) {
		...
		for (int o = 0; o < cellsPerRow; o++) {
			for (int i = 0; i < cellsPerRow; i++) {
				cells[o][i] = new Cell(cellSize*o, cellSize*i, cellSize);
			}
		}
	}
		...
		for (int o = 0; o < cellsPerRow; o++) {
			for (int i = 0; i < cellsPerRow; i++) {
				if (new Random().nextInt(2) == 0) {
					cells[o][i].isAlive = true;
				} else {
					cells[o][i].isAlive = false;
				}
			}
		}
	...
	public void step() {
		...
		for (int o = 0; o < cellsPerRow; o++) {
			for (int i = 0; i < cellsPerRow; i++) {
				livingNeighbors[o][i] = getLivingNeighbors(cells, o, i);
			}
		}
...
for (int o = 0; o < cellsPerRow; o++) {
	for (int i = 0; i < cellsPerRow; i++) {
		cells[o][i].liveOrDie(livingNeighbors[o][i]);
	}
}
		repaint();
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		for (int o = 0; o < cellsPerRow; o++) {
			for (int i = 0; i < cellsPerRow; i++) {
				if (cells[o][i].getX() < e.getX() && cells[o][i].getX()+cellSize > e.getX()) {
					if (cells[o][i].getY() < e.getY() && cells[o][i].getY()+cellSize > e.getY()) {

						if (cells[o][i].isAlive) {
							cells[o][i].isAlive = false;
						} else {
							cells[o][i].isAlive = true;
						}
